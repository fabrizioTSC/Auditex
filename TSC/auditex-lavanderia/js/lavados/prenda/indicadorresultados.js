...
async function getDatosIndicador(fecha,sede,tiposervicio,taller){
    ...
    let responseanios = await get("auditex-lavanderia","lavadoprenda","get-indicador-resultados",{ 
        opcion:1,fecha,sede,tiposervicio,taller
    });

    let responsemeses = await get("auditex-lavanderia","lavadoprenda","get-indicador-resultados",{ 
        opcion:2,fecha,sede,tiposervicio,taller
    });

    let responsesemanas = await get("auditex-lavanderia","lavadoprenda","get-indicador-resultados",{ 
...
function setTableIndicador(data){
    ...
    // PRENDAS
    traprobadas_cant += "<tr style='border-top:2px solid #000' > <th class='border-table thead-sistema-3'>PREND. APROB. 1RA</th>";
...
async function getParetos(opcion,idfamdefecto,fecha,sede,tiposervicio,taller){
    ...
    try{
        ...
        return await  get("auditex-lavanderia","lavadoprenda","get-indicador-resultados-defectos",{
