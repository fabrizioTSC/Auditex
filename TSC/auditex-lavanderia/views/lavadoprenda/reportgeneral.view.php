<?php
    ...
    $_SESSION['navbar'] = "Auditex Lavanderia Prenda - Reporte General";
...
<body>
    ...
    <div class="container-fluid mt-3">
        ...
        <form class="row mb-2" id="frmbusqueda" autocomplete="off">  
            ...
            <div class="col-md-2">
                ...
                <select name="cbosede" id="cbosede" style="width: 100%;" class="custom-select custom-select-sm select2 changescombos"></select>
            </div>

            <div class="col-md-2">
                <label for="">Tipo Servicio</label>
                <select name="cbotiposervicio" id="cbotiposervicio" style="width: 100%;" class="custom-select custom-select-sm select2 changescombos"></select>
            </div>
            ...
            <div class="col-md-2">
                <label for="">&nbsp;</label>
                <a class="btn btn-sm btn-block btn-success font-weight-bold" target="_blank" href="../../../controllers/auditex-lavanderia/lavadoprenda.controller.php?operacion=get-report-general" id="btnexportar">EXPORTAR</a>
            </div>
        ...
        <div class="table-responsive bg-white" style="max-height: 400px;">
            ...
            <table class="table table-bordered table-sm  text-center m-0 table-fichas">
                ...
                <tfoot>
                    <tr>
                        <td colspan="11" classs="text-right font-weight-bold" >TOTAL</td>
                        <td class="font-weight-bold" id="tdficha"></td>
                        <td class="font-weight-bold" id="tdparte"></td>
                        <td class="font-weight-bold" id="tdmuestra"></td>
                        <td colspan="8"></td>
                    </tr>
                </tfoot>
            </table>
    ...
    <script>
        ...
        frmbusqueda.addEventListener('submit',(e)=>{
            ...
            get("auditex-lavanderia","lavadoprenda","getreportegeneral",filtros)
                .then(response =>{
                    ...
                    if(response){
                        ...
                        for(let fila of response){
                            ...
                            totalmuestra += parseFloat(fila["SAMPLESIZE"]);
                            totalficha += parseFloat(fila["CANTOTALFICHA"]);
                            totalparte += parseFloat(fila["CANTIDADPARTIDA"]);
                            ...
                            tr +=  `
                                ...
                                <tr >
                                    ...
                                    <td class="w-bajo" nowrap> ${ format_miles(fila["CANTOTALFICHA"])   } </td>
                                    <td class="w-bajo" nowrap> ${ format_miles(fila["CANTIDADPARTIDA"]) } </td>
                                    <td class="" nowrap>  ${fila["SAMPLESIZE"]} </td>
                                    <td class="" nowrap>  ${fila["RESULTADOFINAL"]} </td>
                                    <td class="" nowrap>  ${ fila["DES_DEFECTOS"] == null ? '' : fila["DES_DEFECTOS"]  } </td>
                                    <td class="w-bajo-4" nowrap>  ${fila["CAN_DEFECTOS"] == null ? '' : fila["CAN_DEFECTOS"] } </td>
                                    <td class="w-bajo-4" nowrap>  ${fila["RECHAZADO"]} </td>
                                    <td class="" nowrap>  ${fila["COD_DEFECTOS"] == null ? '' : fila["COD_DEFECTOS"] } </td>
                        ...
                        $("#tdmuestra").text( format_miles(totalmuestra) );
                        $("#tdficha").text( format_miles(totalficha) );
                        $("#tdparte").text( format_miles(totalparte) );
