<?php
    ...
    $_SESSION['navbar'] = "Eliminar Auditoria Lavanderia - Prenda ";
...
<div class="container mt-3"> 
    ...
    <?php
        ...
        if(isset($_GET["ficha"])){
            ...
            $ficha = $_GET["ficha"];
            $dataficha = $objModelo->getAll("AUDITEX.PQ_MODAUDITEX.SPU_GETFICHASAUDITORIA",[5,$ficha]);
            // var_dump($dataficha);
        }
    ?>
            ...
            <tbody class="bg-white">
                <?php foreach($dataficha as $data): ?>
                    ...
                    <tr>
                        ...
                        <td><?= $data["PARTE"]; ?></td>
                        <td><?= $data["NUMVEZ"]; ?></td>
                        <td><?= $data["CANTOTALFICHA"]; ?></td>
                        <td><?= $data["CANPAR"]; ?></td>
                        <td><?= $data["FECHACIERRE"]; ?></td>
                        <td><?= $data["USUARIOCIERRE"]; ?></td>
                        <td>
                            <button 
                                    class="btn btn-danger btn-sm eliminaraud" 
                                    type="button" 
                                    <?= $disabled;  ?> 
                                    data-idficha="<?= $data["IDFICHA"]; ?>"
    ...
    $(".eliminaraud").click(async function(){
        ...
        if(rpt.value){
            ...
            post("auditex-generales","generales","setdeleteauditorias",[
                5,idficha,ficha,parte,vez,"<?= $_SESSION["user"]  ?>"
            ])
                    .then(response => {
