<?php
	...
	$sql="BEGIN SP_INSP_SELECT_VALNUETUR(:LINEA,:ANTERIOR,:TURNO,:CANTIDAD); END;";
	...
	if ($cantidad==0) {
		$sql="BEGIN SP_INSP_INSERT_LINETOTUR(:LINEA,:ANTERIOR,:TURNO,:CUOTA,:HORINI,:HORFIN,:JORNADA); END;";
		$stmt=oci_parse($conn,$sql);
		oci_bind_by_name($stmt,':LINEA',$_POST['linea']);
		oci_bind_by_name($stmt,':ANTERIOR',$_POST['anterior']);
		oci_bind_by_name($stmt,':TURNO',$_POST['turno']);
