<?php
	...
	$sql="BEGIN SP_INSP_DELETE_LINETOTURHOR(:LINEA,:TURNO,:ANTERIOR); END;";
	...
	for ($i=0; $i < count($array); $i++) {
		$sql="BEGIN SP_INSP_INSERT_LINETOTURHOR(:LINEA,:ANTERIOR,:HORA,:MINASI,:TURNO,:NUMOPE,:MINHOR); END;";
		$stmt=oci_parse($conn,$sql);
		oci_bind_by_name($stmt,':LINEA',$_POST['linea']);
		oci_bind_by_name($stmt,':ANTERIOR',$_POST['anterior']);
	...
	$sql="BEGIN SP_INSP_UPDATE_LINETOTUR(:LINEA,:ANTERIOR,:TURNO,:CUOTA,:HORINI,:HORFIN,:JORNADA,:CAMEST); END;";
	$stmt=oci_parse($conn,$sql);
	oci_bind_by_name($stmt,':LINEA',$_POST['linea']);
	oci_bind_by_name($stmt,':ANTERIOR',$_POST['anterior']);
	oci_bind_by_name($stmt,':TURNO',$_POST['turno']);
	...
	if ($row['CONTADOR']=="0") {
		...
		while($row=oci_fetch_assoc($OUTPUT_CUR)){
			...
			$stmt = oci_parse($conn,'BEGIN SP_MONI_SEL_DATOS_REP_MONITOR6(:LINEA,:FECHA,:HORA, :HORINIDIA, :HORINI, :HORFIN, :FECINI, :FECFIN, :HORACT, :TURNO, :MINASI1, :MINASI2, :MINEFICIENCIA, :PRENDASPRO, :PRENDASPRY, :CUOTA, :CAMBIOEST, :CLIENTE, :MINEFICACIA, :PRENDASDEF, :PRENDASINS, :PRENDASREP, :AUDAPR, :AUDTOT, :NUMINS); END;');	
			...
			$lineas[$i]=$uno;
			$i++;
			$stmt = oci_parse($conn,'BEGIN SP_INSP_INSERT_HISEFILIN(:LINEA,:TURNO,:FECHA,:FECHAFOR,:HORA, :HORINI, :HORFIN, :CLIENTE, :EFICIENCIA, :EFICACIA, :PRENDASPRO, :CUOTA, :PRENDASDEF, :PRENDASINS, :PRENDASREP, :FACTOR, :FLAGCAMBIO,:MINASI,:MINEFI,:MINEFC,:AUDAPR, :AUDTOT); END;');	
