<?php
	...
	if($denominador!=0){
		$mineficiencia=(float)$mineficiencia;
		$uno->ant_eficiencia=round(floatval($mineficiencia)*100/$denominador);
		$uno->ant_eficacia=round(floatval($mineficacia)*100/$denominador);
	}else{
		$uno->ant_eficiencia="-";
		$uno->ant_eficacia="-";
	}
	...
	/////////////////// DATOS DE LINEA 1
	$stmt = oci_parse($conn,'BEGIN SP_MONI_SEL_DATOS_REP_MONITOR5(:LINEA, :HORINIDIA, :HORINI, :HORFIN, :FECINI, :FECFIN, :HORACT, :TURNO, :MINASI1, :MINASI2, :MINEFICIENCIA, :PRENDASPRO, :PRENDASPRY, :CUOTA, :CAMBIOEST, :CLIENTE, :MINEFICACIA, :PRENDASDEF, :PRENDASINS, :PRENDASREP,:AUDAPR,:AUDTOT,:NUMINS); END;');	
	oci_bind_by_name($stmt,':LINEA',$_POST['l1']);
	oci_bind_by_name($stmt,':HORINIDIA',$hinidia,40); 
	oci_bind_by_name($stmt,':HORINI',$hini,40);   
	oci_bind_by_name($stmt,':HORFIN',$hfin,40); 
	...
	$denominador=$rowdos_uno+$rowdos_dos;
	if($denominador!=0){
		$mineficiencia=floatval($mineficiencia);
	...
	if($denominador!=0){
		$mineficiencia=(float)$mineficiencia;
		$dos->ant_eficiencia=round(floatval($mineficiencia)*100/$denominador);
		$dos->ant_eficacia=round(floatval($mineficacia)*100/$denominador);
	}else{
		$dos->ant_eficiencia="-";
		$dos->ant_eficacia="-";
	}
	...
	if($denominador!=0){
		$mineficiencia=floatval($mineficiencia);
		$dos->eficiencia=round($mineficiencia*100/$denominador);
		$dos->eficacia=round($mineficacia*100/$denominador);
	}else{
		$dos->eficiencia="-";
		$dos->eficacia="-";
	}
