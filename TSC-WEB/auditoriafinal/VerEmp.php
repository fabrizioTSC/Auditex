		...
		<div class="bodyContent mainBodyContent">
			...
			<div id="main-auditoria" style="display: none;">
				...
				<div class="forms-content" id="form-1">
					...
					<button class="btnPrimary" style="margin-left: calc(50% - 80px); margin-top: 5px;" onclick="save_form1_dos()" id="btngrachelis">Grabar</button>
				</div>
				...
				<div class="forms-content" id="form-3" style="display: none;">
					...
					<div id="detalle-caja" style="display: none;">
						...
						<div style="display: flex;">
							<label style="padding: 8px 5px 0 0;font-weight: bold;">Resultado</label>
							<select class="classCmbBox" id="rescaj" style="margin-bottom: 0; width: 120px;">
			...
			$.ajax({
				...
				success:function(data){
					...
					if (data.state) {
						if (estado=='T') {
							allow_edit=false;
							if(document.getElementById("btnendaud")){
								document.getElementById("btnendaud").style.display="none";
							}
						}else{
							allow_edit=true;
							if(document.getElementById("btnendaud")){
								document.getElementById("btnendaud").style.display="block";
							}
						}
						fill_cajas(data.cajas);
						fill_cajasaud(data.cajasaud);
			...
			if (!audter_v) {
				if (allow_edit) {
					...
					if (c) {
						...
						$.ajax({
							type:'POST',
							url:'config/addCajErpVerEmpAF.php',
							data:{
								pedido:pedido,
								color:color,
								numvez:numvez,
							...
							'<tr>'+
								...
								'<td>'+
									'<div class="check-content" id="aca-cheblo3-'+data.chklis[i].CODAVIO+'">'+
										'<div class="marker-check anicheblo" onclick="change_check(this)" data-cod="'+data.chklis[i].CODAVIO+'" id="cheblo3-'+data.chklis[i].CODAVIO+'" data-value="0" data-ta="'+data.chklis[i].TIPOAVIO+'">NO</div>'+
									'</div>'+
								'</td>'+
							'</tr>';							
						}						
