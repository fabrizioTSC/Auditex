<?php
	...
	$sql="BEGIN SP_AA_INSERT_FICCORMEDFINACA(:CODFIC,:CODUSU); END;";
	...
	$sql="BEGIN SP_AA_INSERT_FICCORMEDFIN(:CODFIC,:ESTTSC,:NUMPRE); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
	...
	if ($result) {
		...
		$ant_nom="";
		$cont=0;
		$sql="BEGIN SP_AA_SELECT_ESTMEDCOS(:ESTTSC,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':ESTTSC', $_POST['esttsc']);
		...
		$i=0;
		$validador=false;
		$sql="BEGIN SP_AA_SELECT_HEAMEDTAL(:ESTTSC,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':ESTTSC', $_POST['esttsc']);
		$OUTPUT_CUR=oci_new_cursor($conn);
		oci_bind_by_name($stmt, ':OUTPUT_CUR', $OUTPUT_CUR,-1,OCI_B_CURSOR);
