...
$(document).ready(function(){
	$.ajax({
		...
		url:"config/getInfoIndDefCalInt.php",
		success:function(data){
			...
			for (var i = 0; i < data.defectos.length; i++) {
				...
				'<div class="lineBody" onclick="detalle_defecto(\''+data.defectos[i]['CODDEF']+'\',\''+data.defectos[i]['DESDEF']+'\')">'+
					...
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);">'+formatNumber(data.defectos[i]['CANMUE'])+'</div>'+
	...
	$("#idNumAnio").change(function(){
		...
		$.ajax({
			...
			url:"config/getSemanasIndDefCalInt.php",
			success:function(data){
				console.log(data);
				var html='';
				for (var i = 0; i < data.semanas.length; i++) {
	...
	$("#idNumSem").change(function(){
		...
		$.ajax({
			...
			url:"config/getNewInfoIndDefCalInt.php",
			success:function(data){
				console.log(data);
				var html='';
				for (var i = 0; i < data.defectos.length; i++) {
...
function detalle_defecto(coddef,desdef){
	...
	$.ajax({
		...
		url:"config/getInfoIndDefXCodCalInt.php",
		success:function(data){
			console.log(data);
...
function downloadPDF(){
	$.ajax({
	  	...
	  	success:function(data){
	  		...
	  		a.target="_blank";
	  		a.href="fpdf/crearPdfIndDefCalInt.php?n="+data+
	  		"&t="+title+"&t2="+title2+
