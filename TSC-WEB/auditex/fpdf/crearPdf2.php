...
$sql="BEGIN SP_AT_INDICADOR_RESULTADOSAUX(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:OPCION,:OUTPUT_CUR); END;";
...
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("INDICADORES DE RESULTADOS DE AUDITORÍA FINAL DE COSTURA"),0,0,'C');
$pdf->Ln();
...
for ($i=0; $i < count($por_apr); $i++) { 
	if ($i==0) {
		...
		if ($porcen<intval($ran2)) {			
			$pdf->SetFillColor(220,20,30);
		}else{
			if($porcen>=intval($ran1)){
				$pdf->SetFillColor(90,195,40);
			}else{
				$pdf->SetFillColor(220,160,10);				
			}
...
$i=0;
$sumDefectosU=0;
$sql="BEGIN SP_AT_INDICADOR_RESULTADOSAUX2(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:CODFAM,:OPCION,:OUTPUT_CUR); END;";
$stmt=oci_parse($conn, $sql);
oci_bind_by_name($stmt, ':CODTLL', $_GET['ct']);
oci_bind_by_name($stmt, ':CODSED', $_GET['cs']);
oci_bind_by_name($stmt, ':CODTIPSER', $_GET['cts']);
