...
function fill_defectos(defectos,sumtot){
	var html='';
...
function detalle_defecto_taller(codtll_new,destll){
	...
	$.ajax({
		...
		success:function(data){
			...
			var max_per=0;
			for (var i = 0; i < data.anios.length; i++) {
				var porcentaje=Math.round(Math.round(data.anios[i]['SUMCODDEF']*10000)/data.anios[i]['CANMUE'])/100;
				...
				'<div class="divanios[i]">'+
					...
					'<div class="itemhs1 items4">'+formatNumber(data.anios[i]['CANMUE'])+'</div>'+
			...
			for (var i = 0; i < data.meses.length; i++) {
				var porcentaje=Math.round(Math.round(data.meses[i]['SUMCODDEF']*10000)/data.meses[i]['CANMUE'])/100;
				if(porcentaje>max_per){
					max_per=porcentaje;
				}
				html+=
				'<div class="divanios[i]">'+
					...
					'<div class="itemhs1 items4">'+formatNumber(data.meses[i]['CANMUE'])+'</div>'+
			...
			var html='';
			for (var i = 0; i < data.semanas.length; i++) {
				var porcentaje=Math.round(Math.round(data.semanas[i]['SUMCODDEF']*10000)/data.semanas[i]['CANMUE'])/100;
				if(porcentaje>max_per){
					max_per=porcentaje;
				}
				html+=
				'<div class="divanios[i]">'+
					...
					'<div class="itemhs1 items4">'+formatNumber(data.semanas[i]['CANMUE'])+'</div>'+
