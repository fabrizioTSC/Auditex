	...
	$.ajax({
		...
		success:function(data){
			...
			for (var i = 0; i < data.semanas.length; i++) {
				var PorAudApro=0;
				if(data.semanas[i]['AUD_IND']!=0){
					PorAudApro=Math.round(data.semanas[i]['AUD_APR1']*10000/data.semanas[i]['AUD_IND'])/100;
				}
				var styleC="colorB";
				var color=window.chartColors.yellow;
				if (PorAudApro>=param_codran1) {
				...
				var PorPreApro=0;
				if (data.semanas[i]['PREN_IND']!=0) {
					PorPreApro=Math.round(data.semanas[i]['PREN_APR1']*10000/data.semanas[i]['PREN_IND'])/100;
				}
				var por_rec=Math.round((100-PorAudApro)*100)/100;
				html+=
				'<div class="divanios[i]">'+
					...
					'<div class="itemhs1 items4 '+styleC+'">'+PorAudApro+'%</div>'+
					'<div class="itemhs1 items4">'+por_rec+'%</div>'+
					'<div class="itemhs1 items3"></div>'+
