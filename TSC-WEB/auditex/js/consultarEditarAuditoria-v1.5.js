...
$(document).ready(function(){
	...
	$(".btnBuscarSpace").click(function(){
		...
		if (codfic=="") {
			...
			$("#table-pendientes").css("display","none");
			...
			$.ajax({
				type:"POST",
				url:"config/getFichaConsultarEditar.php",
				data:{
					codfic:codfic,
					typeRequest:2
				},
				success:function(data){
					...
					console.log(data);
					if (data.ficpen.length!=0) {
						let html_aux='';
						for (var i = 0; i < data.ficpen.length; i++) {
							html_aux+=
								'<div class="tblLine">'+
									'<div class="itemBody" style="width: 15%;">'+data.ficpen[i].PARTE+'</div>'+
									'<div class="itemBody" style="width: 15%;">'+data.ficpen[i].NUMVEZ+'</div>'+
									'<div class="itemBody" style="width: 15%;">'+data.ficpen[i].CANPAR+'</div>'+
									'<div class="itemBody" style="width: 55%;">'+data.ficpen[i].DESTLL+'</div>'+
								'</div>';
						}
						$("#body-table-pendientes").empty();
						$("#body-table-pendientes").append(html_aux);
						$("#table-pendientes").css("display","block");
					}
					if (data.state==true) {
						if (data.fichas.length==0) {
							...
							for (var i = 0; i < data.fichas.length; i++) {
								...
								htmlFichas += `
									...
									${data.fichas[i].NUMVEZ},
									${data.fichas[i].PARTE},
									'${data.fichas[i].TIPOAUDITORIA}')">
								`;

								htmlFichas+=
									'<div class="itemBody" style="width: 20%;">'+data.fichas[i].CODUSU+'</div>'+
									'<div class="itemBody" style="width: 20%;">'+data.fichas[i].DESTLL+'</div>'+
									'<div class="itemBody" style="width: 10%;">'+data.fichas[i].PARTE+'</div>'+
									...
									'<div class="itemBody" style="width: 10%;">'+data.fichas[i].FECINIAUD+'</div>'+
									'<div class="itemBody" style="width: 10%;">'+data.fichas[i].TIPOAUDITORIA+'</div>'+
									'<div class="itemBody" style="width: 10%;">'+resul+'</div>'+
								'</div>';
							}
...
$("#idTblFichas").on('click','.tblLine',function(){
	// alert("click");
	$(".tblLine").removeClass("seleccionado");
...
function fichaSelectedForConsulta(codfic,aql,codaql,codtad,numvez,parte,tipoauditoria){
	...
	fsfcnumvez			=	numvez;
	fsfcparte			=	parte;
	fscstipoauditoria 	=	(tipoauditoria == "ESPECIAL" || tipoauditoria ==  'E') ? 'E' : 'N';
	...
	$.ajax({
		...
		data:{
			...
			codaql:codaql,
			requestFicha:"0",
			fichatipoauditoria:fscstipoauditoria
		},
		success:function(data){
			console.log(data);
			if(data.state==true){
				...
				if (data.partida.DESTLL!=undefined) {
					...
					'<a href="../AuditoriaFinalCorte/ConsultarEditarAuditoria.php?solocorte=1&codfic='+codfic+'">'+data.partida.DESTLL+'</a>';
				}else{
					document.getElementById("idNombreTalCor").innerHTML="";
				}			
				...
				// OBSERVACIONES
				document.getElementById("observacionesnuevo").innerHTML =  data.ficha.OBSERVACIONES;
		...
		success:function(data){
			...
			for (var i = 0; i < data.clarec.length; i++) {
				...
				'<div class="sameline" style="width: 100%;margin-bottom: 5px;">'+
					...
					'<div class="lbl" style="margin: 0px;width: 40%;font-size: 12px;">'+data.clarec[i].DESCLAREC+'</div>'+
					'<input type="number" data-idClaRec="'+data.clarec[i].CODCLAREC+'" class="iptNumber classClaRec" style="width: 20%;margin: 0px;" value="'+data.clarec[i].CANPRE+'">'+

					'<div class="lbl" style="margin: 0px;width: 5%;font-size: 12px;"></div>'+


					'<input type="text" placeholder="Observación" data-idClaRec="'+data.clarec[i].CODCLAREC+'" class="iptNumber classClaRecObs" style="width: 35%;margin: 0px;"  value="'+data.clarec[i].OBSERVACION+'" >'+
...
$(".classClaRec").keyup(function(){
	...
	for (var i = 0; i < arrayClaRec.length; i++) {
		...
		// aux.push(arrayClaRec[i].value == "" ? "0" : arrayClaRec[i].value);

		// OBSERVACION
		// aux.push(arrayClaRecObs[i].value == "" ? "" : arrayClaRecObs[i].value.trim());


		sumtotal+=parseInt(arrayClaRec[i].value == "" ? "0" : arrayClaRec[i].value);
