...
function downloadPDF(){
	...
	var numimg=document.getElementsByClassName("contentGraph").length;
	for (var i = 0; i < numimg; i++) {
		$.ajax({
		  	type: "POST",
		  	url: "config/saveTmpImg-RepDesMed.php",
		  	data: { 
		    	imgBase64: document.getElementById("chart-area-"+(i+1)).toDataURL("image/png"),
		    	numimg:(i+1),
				esttsc:esttsc,
				codfic:codfic
		  	},
	...
	setTimeout(go_download,1000);
}

function go_download(){
	$(".panelCarga").fadeOut(100);
	var numimg=document.getElementsByClassName("contentGraph").length;
	var link="fpdf/pdfRepDesMed.php?esttsc="+esttsc+"&codfic="+codfic+"&fecini="+fecini+"&fecfin="+fecfin+"&numimg="+numimg
	+"&estcli="+document.getElementById("idestcli").innerHTML
	+"&cliente="+document.getElementById("idcliente").innerHTML;
