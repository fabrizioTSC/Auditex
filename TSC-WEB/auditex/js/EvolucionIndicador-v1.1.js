...
$(document).ready(function(){
	$.ajax({
		...
		success:function(data){
			...
			if (type==0) {
				$("#titulodetalle").append(data.titulo+" / "+num+" semanas");
			}else{
				$("#titulodetalle").append(data.titulo+" / "+num+" meses");
			}
			...
			for (var i = 0; i < data.semanas.length; i++) {
				...
				var headertitle='';
				if (type==0) {
					headertitle='S. '+data.semanas[i]['NUMERO_SEMANA'];
				}else{
					headertitle=proceMes(data.semanas[i]['MES']);
				}
				var PorPreApro=Math.round(data.semanas[i]['PREN_APR']*10000/data.semanas[i]['PREN_TOT'])/100;
				html+=
				'<div class="divanios[i]">'+
					'<div class="itemhs1">'+headertitle+'</div>'+
...
function downloadPDF(){
	$.ajax({
	  	...
	  	success:function(data){
	  		...
	  		a.target="_blank";
	  		a.href="fpdf/crearPdf2.php?n="+data+
