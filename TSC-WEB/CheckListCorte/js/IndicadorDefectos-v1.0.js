		...
		success:function(data){
			...
			for (var i = 0; i < data.defectos.length; i++) {
				...
				'<div class="lineBody" onclick="detalle_defecto(\''+data.defectos[i]['CODDEF']+'\',\''+data.defectos[i]['DESDEF']+'\')">'+
					'<div class="itemhs1 items4" style="width: calc(70% - 10px);text-align:left;">'+(i+1)+'. '+data.defectos[i]['DESDEF']+'</div>'+
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);">'+formatNumber(data.defectos[i]['SUMDEF'])+'</div>'+
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);">'+parseInt(parseInt(data.defectos[i]['SUMDEF'])*10000/parseInt(data.sumtot))/100+'%</div>'+
				'</div>';
			}
