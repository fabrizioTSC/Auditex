...
/*
	...
	$sql="BEGIN SP_AFC_INSERT_FICCORMEDFIN(:CODFIC,:NUMPRE); END;";
	...
	$ant_nom="";
	$cont=0;
	$sql="BEGIN SP_AFC_SELECT_ESTILOMEDIDA(:CODFIC,:OUTPUT_CUR); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
	...
	$i=0;
	$validador=false;
	$sql="BEGIN SP_AFC_SELECT_FICCORMEDFIN(:CODFIC,:OUTPUT_CUR); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
	$OUTPUT_CUR=oci_new_cursor($conn);
	oci_bind_by_name($stmt, ':OUTPUT_CUR', $OUTPUT_CUR,-1,OCI_B_CURSOR);
	...
	$sql="BEGIN SP_AFC_INSERT_FICCORMEDFIN_JV2(:CODFIC,:NUMPRE,:HILO,:TRAVEZ,:LARGMANGA); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
	oci_bind_by_name($stmt, ':NUMPRE', $_POST['cantidad']);
	...
	if ($result) {
		...
		$ant_nom="";
		$cont=0;
		$sql="BEGIN SP_AFC_SELECT_ESTILOMEDIDA_J(:CODFIC,:HILO,:TRAVEZ,:LARGMANGA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		...
		$i=0;
		$validador=false;
		$sql="BEGIN SP_AFC_SELECT_FICCORMEDFIN_J(:CODFIC,:HILO,:TRAVEZ,:LARGMANGA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		oci_bind_by_name($stmt, ':HILO', $_POST['hilo']);
		oci_bind_by_name($stmt, ':TRAVEZ', $_POST['travez']);
