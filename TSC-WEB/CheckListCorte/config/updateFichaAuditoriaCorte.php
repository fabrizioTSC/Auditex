<?php
	...
	$sql="BEGIN SP_AFC_SELECT_FICAUDCOR(:CODFIC,:NUMVEZ,:PARTE,:CODTAD,:CODAQL,:OUTPUT_CUR); END;";
	...
	if(is_null($rowAux['CODUSU'])){
		$sql="BEGIN SP_AFC_UPDATE_FICAUDCOR(:CODFIC,:NUMVEZ,:PARTE,:CODTAD,:CODAQL,:TIPAUD,:NEWCAN); END;";
		$stmt=oci_parse($conn,$sql);
		oci_bind_by_name($stmt,':CODFIC', $_POST['codfic']);
		oci_bind_by_name($stmt,':NUMVEZ', $_POST['numvez']);
		oci_bind_by_name($stmt,':PARTE', $_POST['parte']);
		...
		if($result){			
			...
			/*$sql="BEGIN SP_AFC_UPDATE_INIFIC(:CODFIC,:CODUSU); END;";
			$stmt=oci_parse($conn,$sql);
			oci_bind_by_name($stmt,':CODFIC', $_POST['codfic']);
