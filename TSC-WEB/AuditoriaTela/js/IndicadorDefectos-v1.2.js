		...
		success:function(data){
			...
			for (var i = 0; i < data.defectos.length; i++) {
				...
				'<div class="lineBody" onclick="detalle_defecto(\''+data.defectos[i]['CODDEF']+'\',\''+data.defectos[i]['DESDEF']+'\')">'+
					...
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);text-align:left;">'+data.defectos[i]['CANMUE']+'</div>'+
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);">'+formatNumber(data.defectos[i]['SUMDEF'])+'</div>'+
					'<div class="itemhs1 items4" style="width: calc(15% - 10px);">'+parseInt(parseInt(data.defectos[i]['SUMDEF'])*100000/parseInt(data.defectos[i]['CANMUE']))/100+'</div>'+
				'</div>';
			}
...
/*detalle_defecto(coddef,desdef){
	window.location.href="IndicadorDefecto.php?coddef="+
	coddef+"&desdef="+desdef+"&codtll="+codtll+"&codsede="+codsede+"&codtipser="+codtipser+"&titulo="+document.getElementById("titulodetalle").innerHTML;
}*/
...
function detalle_defecto(coddef,desdef){
	...
	let fecha = tipo == 1 ? null :  $("#fecfin").val();
	...
	$.ajax({
		...
		success:function(data){
			...
			var max_per=0;
			for (var i = 0; i < data.anios.length; i++) {
				var porcentaje=Math.round(Math.round(data.anios[i]['SUMCODDEF']*100000)/data.anios[i]['SUMCANMUE'])/100;
			...
			for (var i = 0; i < data.meses.length; i++) {
				var porcentaje=Math.round(Math.round(data.meses[i]['SUMCODDEF']*100000)/data.meses[i]['SUMCANMUE'])/100;
				if(porcentaje>max_per){
					max_per=porcentaje;
				}
				html+=
			...
			var html='';
			for (var i = 0; i < data.semanas.length; i++) {
				var porcentaje=Math.round(Math.round(data.semanas[i]['SUMCODDEF']*100000)/data.semanas[i]['SUMCANMUE'])/100;
				if(porcentaje>max_per){
					max_per=porcentaje;
				}
				html+=
...
function downloadPDF(){
	$.ajax({
	  	...
	  	success:function(data){
	  		...
	  		"&t="+title+"&t2="+title2+
	  		"&cpv="+codprv+"&cc="+codcli+"&cp="+codpro+"&cd="+cd;
	  		a.click();
	  	}
	}).done(function(o) {
...
function descargar_inddef(){
	var a=document.createElement("a");
	a.target="_blank";
	a.href="config/exports/exportIndDef.php?codprv="+codprv+"&codcli="+codcli+"&codpro="+codpro
