...
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("ANÁLISIS DE DEFECTOS DE TELA ACABADA"),0,0,'C');
$pdf->Ln();
...
oci_bind_by_name($stmt, ':CODPRV', $_GET['cpv']);
oci_bind_by_name($stmt, ':CODCLI', $_GET['cc']);
oci_bind_by_name($stmt, ':CODPRO', $_GET['cp']);
$opcion=0;
...
while ($row=oci_fetch_assoc($OUTPUT_CUR)) {
	...
	$num_kg_mue[$l]=number_format($row['SUMCANMUE']);
	$num_def_tot[$l]=number_format($row['SUMDEF']);
	$por_def[$l]=(round($row['SUMCODDEF']*100000/$row['SUMCANMUE'])/100);
	$l++;
}
...
$pdf->Ln();
for ($i=0; $i < count($num_kg_mue); $i++) { 
	if ($i==0) {
		$pdf->Cell($first_col_width,6,$num_kg_mue[$i],1,0,'L',true);
	}else{
		$pdf->Cell($width_by_cell,6,$num_kg_mue[$i],1,0,'R');
	}
}
$pdf->Ln();
...
$pdf->Output('D','Análisis de Defectos de Tela Acabada - '.str_replace("/","-",$_GET['t']).'.pdf',true);
