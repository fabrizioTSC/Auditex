	...
	if ($validar>0) {
		...
		if (isset($_POST['array'])) {
			...
			for ($i=0; $i < count($array); $i++) {
				$sql="BEGIN SP_AUDTEL_INSERT_PARAUDTONV2(:PARTIDA, :CODTEL, :CODPRV, :CODTAD, :NUMVEZ, :PARTE, :CODTON, :RESTSC,:CODREC1,:CODREC2, :PERUSU); END;";
				$stmt=oci_parse($conn, $sql);
				oci_bind_by_name($stmt, ':PARTIDA', $_POST['partida']);
				oci_bind_by_name($stmt, ':CODTEL', $_POST['codtel']);
				oci_bind_by_name($stmt, ':CODPRV', $_POST['codprv']);
		...
		if ($cont==0) {
			...
			$sql="BEGIN SP_AUDTEL_UPDATE_PARAUD(:PARTIDA, :CODTEL, :CODPRV, :CODTAD, :NUMVEZ, :PARTE, :RESULTADO, :CODUSU, :PERUSU,:OBS,:CODRES); END;";
			$stmt=oci_parse($conn, $sql);
			oci_bind_by_name($stmt, ':PARTIDA', $_POST['partida']);
			oci_bind_by_name($stmt, ':CODTEL', $_POST['codtel']);
			oci_bind_by_name($stmt, ':CODPRV', $_POST['codprv']);
