...
function cargarDatos(codFic,tipoMuestra,numMuestra,numvez,parte,codtad,codaql,codusu,codusuario){
	...
	$.ajax({
		...
		url:"config/getDetalleFichaAPC.php",
		success:function(data){
			...
			if (data.state==true) {
				...
				for (var i = 0; i < data.obs.length; i++) {
					html+=
					'<div class="sameline" style="border-bottom: 1px solid #333;">'+
						'<div style="width:calc(100% - 54px);">'+data.obs[i].SEC+'. '+data.obs[i].OBS+'</div>'+
						'<div style="width:calc(54px);display:flex;">'+
							'<button style="width:27px;" onclick="edit_obs('+data.obs[i].SEC+',\''+data.obs[i].OBS+'\')"><i class="fa fa-pencil" aria-hidden="true"></i></button>'+
							'<button style="width:27px;" onclick="delete_obs('+data.obs[i].SEC+')"><i class="fa fa-trash" aria-hidden="true"></i></button>'+
						'</div>'+
					'</div>';
				}
				...
				if (data.DESCEL=="") {
					...
					document.getElementById("idNombreTaller").innerHTML=data.fichas[0].DESTLL+" - "+data.DESCEL;
				}
				document.getElementById("idPedido").innerHTML=data.fichas[0].PEDIDO;
				document.getElementById("idEsttsc").innerHTML=data.fichas[0].ESTTSC;
				document.getElementById("idEstcli").innerHTML=data.fichas[0].ESTCLI;
...
function add_numdef(coddef){
	...
	$.ajax({
		type:"POST",
		url:"config/updateDefectoAPC.php",
		data:{
			codfic:codFicha,
			coddef:coddef,
			codope:0,
...
function terminarAuditoraFicha(){
	...
	$.ajax({
		...
		url:"config/changeStateFichaAPC.php",
		success:function(data){
			//console.log(data);
			if(data.state==true){
				window.location.href="AudProCorRegistrada.php?codfic="+codFicha+
...
function RegistrarMedidas(){
	RegistrarMedidas_Integracion(codFicha,"AuditarMedidas","DivContenido");

	// $(".panelCarga").fadeIn(100);
	// $.ajax({
	// 	url:'config/getEncogimiento.php',
	// 	type:'POST',
...
function save_observacion(){
	if ($("#idTextObs").val().length<max_chars) {
		$(".panelCarga").fadeIn(100);
		$.ajax({
			type:"POST",
			data:{
			...
			url:"config/saveObservacionFicAPCR.php",
			success:function(data){
				console.log(data);
				var html='';
				if(!data.state){
					...
					html+=
					'<div class="sameline" style="border-bottom: 1px solid #333;">'+
						'<div style="width:calc(100% - 54px);">'+numobs+'. '+$("#idTextObs").val()+'</div>'+
						'<div style="width:calc(54px);display:flex;">'+
							'<button style="width:27px;" onclick="edit_obs('+numobs+',\''+$("#idTextObs").val()+'\')"><i class="fa fa-pencil" aria-hidden="true"></i></button>'+
							'<button style="width:27px;" onclick="delete_obs('+numobs+')"><i class="fa fa-trash" aria-hidden="true"></i></button>'+
						'</div>'+
					'</div>';
					$("#tblObservaciones").append(html);
				}
		...
		alert("El texto debe ser menor a "+max_chars+" caracteres!");
	}
}
...
function save_edit_observacion(){
	if ($("#idTextObsEdit").val().length<max_chars) {		
		$(".panelCarga").fadeIn(100);
		$.ajax({
			type:"POST",
			data:{
