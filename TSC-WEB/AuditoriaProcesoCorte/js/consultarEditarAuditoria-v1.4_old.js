...
$(document).ready(function(){
	$(".btnBuscarSpace").click(function(){
		...
		if (codfic=="") {
			...
			$.ajax({
				type:"POST",
				url:"config/getFichaConsultaAPC.php",
				data:{
					codfic:codfic
				},
...
function editarDefecto(action){
	...
	$.ajax({
		type:"POST",
		url:"config/gestionDefectosAPC.php",
		data:{			
			option:action,
			codfic:fsfccodfic,
			codtad:fsfccodtad,
...
function guardarDefecto(){
	...
	$.ajax({
		type:"POST",
		url:"config/saveDefectoNuevoAPC.php",
		data:{						
			codfic:fsfccodfic,
			codtad:fsfccodtad,
			numvez:fsfcnumvez,
...
function fichaSelectedForConsulta(codfic,aql,codaql,codtad,numvez,parte){
	...
	$.ajax({
		type:"POST",
		url:"config/getFicConDetAPC.php",
		data:{
			codfic:codfic,
			codtad:codtad,
			numvez:numvez,
		...
		success:function(data){
			...
			if(data.state==true){
				...
				for (var i = 0; i < data.obs.length; i++) {
					html+=
					'<div class="sameline" style="border-bottom: 1px solid #333;" id="lineobs'+data.obs[i].SEC+'">'+
		...
		$.ajax({
			...
			success:function(data){
				...
				if(!data.state){
					...
					numobs++;
					html+=
					'<div class="sameline" style="border-bottom: 1px solid #333;" id="lineobs'+numobs+'">'+
