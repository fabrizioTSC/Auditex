<?php
	...
	/*$sql="BEGIN SP_APCR_VAL_FICINFTEL(:CODFIC,:CONTADOR); END;";
	...
	$fichas=[];
	$i=0;
	$sql="BEGIN SP_APCR_SELECT_FICHAXTALLER(:CODFIC,:NUMVEZ,:PARTE,:CODTAD,:CODAQL,:OUTPUT_CUR); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt,':CODFIC',$_POST['codfic']);
	...
	if ($i==0) {			
		...
		$sql="BEGIN SP_APCR_SELECT_DEFECTOS(:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		$OUTPUT_CUR=oci_new_cursor($conn);
		oci_bind_by_name($stmt,':OUTPUT_CUR',$OUTPUT_CUR,-1,OCI_B_CURSOR);
		$result=oci_execute($stmt);
