<?php
    ...
    $path="No hay estilo!";
	$sql="BEGIN SP_APCR_ENCOGIMIENTOS(:CODFIC,:OUTPUT_CUR); END;";
	...
	while($row=oci_fetch_assoc($OUTPUT_CUR)){
        ...
        if ($row["CANT_MED_AUD"]!=0) {
            $path.=
			"<td><a href='AuditarMedidas.php?codfic=$codigoficha&hilo=$hilo&travez=$travez&largmanga=$largmanga'>ver aud/tot:" .$row["CANT_MED_AUD"]."/".$row["CANT_MED"]."</a></td>";            "<td>".str_replace(",",".",$row['LARGMANGA'])."</td>";
        }else{
            $path.=
            "<td>aud/tot: " .$row["CANT_MED_AUD"]."/".$row["CANT_MED"]."</td>";            
        }		
        //$path.=
        //    "<td><a href='AuditarMedidas.php?codfic=$codigoficha&hilo=$hilo&travez=$travez&largmanga=$largmanga'>ver</a></td>";
        $path.=
        "</tr>";
    }
    $path.=
