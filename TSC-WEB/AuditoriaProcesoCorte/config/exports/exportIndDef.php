...
header("Expires: 0");
$filename = "Indicador_defectos-Auditoria_proceso_corte.xls";
...
if ($_GET['tipo']=="1") {
	...
	$sql="BEGIN SP_APCR_INDICADOR_DEFECTOS_RF(:CODTLL,:CODSED,:CODTIPSER,:OUTPUT_CUR,:FECINI,:FECFIN); END;";
	$stmt=oci_parse($conn, $sql);
	oci_bind_by_name($stmt, ':CODTLL', $_GET['codtll']);
	oci_bind_by_name($stmt, ':CODSED', $_GET['codsede']);
	oci_bind_by_name($stmt, ':CODTIPSER', $_GET['codtipser']);
...
<table>
	...
	<tbody>
		...
		<tr>
			...
			<td style="border:1px solid #333;"><?php echo str_replace(",",".",$row['PORDEF']);?></td>
		</tr>
		<?php
				$i++;
			}
