<?php
	...
	$sql="BEGIN SP_APCR_UPDATE_FICHA3(:CODFIC,:NUMVEZ,:PARTE,:CODTAD,:CODAQL,:CODUSU,:RESULTADO,:CANDEF); END;";
	...
	if($result){				
		if ($_POST['resultado']=="R") {
			$sql="BEGIN SP_APCR_INSERT_FICAUDCORREC(:CODFIC,:NUMVEZ,:PARTE,:CODTAD,:CODAQL); END;";
			$stmt=oci_parse($conn, $sql);
			oci_bind_by_name($stmt,':CODFIC',$_POST['codfic']);
			oci_bind_by_name($stmt,':NUMVEZ',$_POST['numvez']);
			oci_bind_by_name($stmt,':PARTE',$_POST['parte']);
