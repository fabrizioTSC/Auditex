...
while($row=oci_fetch_assoc($OUTPUT_CUR)){
	...
	for ($l=0; $l < 4; $l++) {
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetTextColor(0,0,0);
		$pdf->Cell($array_space[1],$height_row,'117 (Abertura de cuello de cost. a cost.)','LTRB',0,'C',true);
		$start_y+=$height_row;
		$pdf->SetXY($start_x,$start_y);
		$pdf->SetFont('Arial','B',7);
		$pdf->SetFillColor(0,0,0);
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetTextColor(0,0,0);
		$pdf->Cell($array_space[1],$height_row,'2X','LTRB',0,'C',true);
		$start_y+=$height_row;
		$pdf->SetXY($start_x,$start_y);
		$pdf->SetFont('Arial','B',7);
		$pdf->SetFillColor(0,0,0);
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetTextColor(0,0,0);
		$pdf->Cell($array_space[1],$height_row,'9','LTRB',0,'C',true);
		$start_y+=$height_row;
		$pdf->SetXY($start_x,$start_y);
		$pdf->SetFont('Arial','B',7);
		$pdf->SetFillColor(0,0,0);
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetTextColor(0,0,0);
		$pdf->Cell($array_space[1],$height_row,'+1/2 -1/2','LTRB',0,'C',true);
		$start_y+=$height_row;
		$pdf->SetXY($start_x,$start_y);
		$height_row=2.5;
		$pdf->SetFont('Arial','B',5);
		...
		for ($i=0; $i < count($colores); $i++) { 
			...
			$pdf->Cell($array_space[1]/count($colores),$height_row*3,'','LTRB',0,'C',true);
			$pdf->SetXY($x,$y);
			$array=get_array_colmed($colores[$i]);
			if (isset($array[0])) {
				$pdf->Cell($array_space[1]/count($colores),$height_row,$array[0],'',0,'C');
			}
			$pdf->SetXY($x,$y+$height_row);
		...
		$pulgadas=['+1 1/2','+1 3/8','+1 1/4','+1 1/8','+1','+7/8','+3/4','+5/8','+1/2','+3/8','+1/4','+1/8','0'
					,'-1/8','-1/4','-3/8','-1/2','-5/8','-3/4','-7/8','-1','-1 1/8','-1 1/4','-1 3/8','-1 1/2'];
		for ($i=0; $i < count($pulgadas); $i++) {
			$pdf->SetFont('Arial','B',5);
			$pdf->SetFillColor(0,0,0);
			$pdf->SetTextColor(255,255,255);
			$pdf->Cell($array_space[0],$height_row,$pulgadas[$i],'LTRB',0,'C',true);
			//Detalle
			if ($pulgadas[$i]=="0") {
				$pdf->SetFillColor(210,210,210);
				$pdf->SetTextColor(0,0,100);
			...
			for ($k=0; $k < count($colores); $k++) { 
				for ($j=0; $j < $numpre; $j++) {
					...
					$pdf->Cell($tamrowmark,$height_row,$text,'LTRB',0,'C',true);
				}
			}
