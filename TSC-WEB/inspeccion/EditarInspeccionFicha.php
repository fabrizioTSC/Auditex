...
<body>
		...
		<div class="mainHeaderPage">
			<div class="bigitemTable">
				<div id="detailInspeccion">
					<div class="sameline">
						<div class="lblInTable">N°:</div>
						<span id="idCodIns"><?php echo $codinscos_aux; ?></span>
						&nbsp;&nbsp;-&nbsp;&nbsp;
			...
			<div class="contentTblInspec" id="tblToAnimate">
				<div class="placeOperations" id="freeOperations">
					<?php						
						$sql="select ic.Canpre,ic.canpredef,ae.esttsc,ae.codfic,ae.codtll from inscos ic
							inner join fichaauditoria fa
							on ic.codfic=fa.codfic
							inner join auditoriaenvio ae
							on fa.codenv=ae.codenv
							where ic.codinscos=".$_GET['codinscos'];
					...
					<div class="itemsFinalTbl spaceToHide" style="width: 192px;display: flex;">
						...
						<div style="width: calc(50% - 5px);padding-left: 5px;">
							...
							<div class="classToInpt" style="width: calc(100% - 12px);height: 35px;font-size: 20px;" onclick="addTotalValue('idNumPreDefNumNew')">
								<div class="tittletext" id="idNumPreDefNumNew" style="padding-top: 7px;"><?php echo $rowAux['CANPREDEF']; ?></div>
				...
				<div class="placeDefectos">
					...
					<?php 
						$sql="SELECT * FROM defecto defe 
							inner join familiadefecto fd on defe.CODFAM=fd.CODFAMILIA 
							WHERE defe.coddef in (
							  select distinct tbl.coddef from 
							  (
							    SELECT coddef FROM defecto defe 
							    inner join familiadefecto fd on defe.CODFAM=fd.CODFAMILIA 
							    WHERE defe.ESTADO='A' and ordenins!=0
							    union all
							    select coddef from detinscos where codinscos=".$_GET['codinscos']."
							  ) tbl
							)
					...
					<div class="spaceFlex" id="spaceToBtn">
						<?php
							$sql="SELECT * FROM detinscos where codinscos=".$_GET['codinscos'];
							$stmt=oci_parse($conn, $sql);
							$result=oci_execute($stmt);
							$dic=[];
							$i=0;
							while($rowdic=oci_fetch_array($stmt)){
								$obj=new stdClass();
								$obj->CODOPE=$rowdic['CODOPE'];
								$obj->CODDEF=$rowdic['CODDEF'];
								$obj->CANDET=$rowdic['CANDET'];
								$dic[$i]=$rowdic;
								$i++;
							}
						...
						?>
							...
							<?php
								for ($j=0; $j < count($operaciones) ; $j++) {
									...
									$validate=false;
									$cantidad=0;
									while($validate==false && $k <count($dic)) {
										if ($dic[$k]['CODOPE']==$operaciones[$j]['CODOPE'] && $dic[$k]['CODDEF']==$defectos[$i]['CODDEF']) {
											$validate=true;
											$cantidad=$dic[$k]['CANDET'];
										}
										$k++;
									}
							?>	
									<div class="itemsFinalTbl maxHeight counterBtn" style="width: 88px;">
										<div class="divBtnsAddMinus" data-def="<?php echo $defectos[$i]['CODDEF']; ?>"
											...
											id="<?php echo 'OPE'.$operaciones[$j]['CODOPE'].'DEF'.$defectos[$i]['CODDEF']; ?>"><?php echo $cantidad; ?></div>
	...
	<script type="text/javascript">
		...
		var codfic="<?php echo $rowAux['CODFIC']; ?>";
		var turno="";
		var codtll="<?php echo $rowAux['CODTLL']; ?>";
		var codinscos_var="<?php echo $codinscos_aux; ?>";
	</script>
	<script type="text/javascript" src="js/inspeccion-v1.7.js"></script>
	<script type="text/javascript" src="js/animacionesGlobales.js"></script>
	<script type="text/javascript" src="js/editInspect-v1.1.js"></script>
</body>
</html>