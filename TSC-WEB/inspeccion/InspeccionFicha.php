	...
	<div class="mainContent">
		...
		<div class="mainHeaderPage">
			<div class="bigitemTable">
				...
				<label for="" id="lblcantidadreturn" style="font-weight: bold;" >Cantidad: 0</label>
			...
			<div class="contentTblInspec" id="tblToAnimate">
				<div class="placeOperations" id="freeOperations">
					...
					<?php 
						$esttsc = $_GET['esttsc'];
						$estilo = explode("-",$esttsc)[0];


						$responseDataSIGEOperaciones = $objModelo->getAllSQLSIGE("UP_REP_CFTARIFADO_OPERACIONES",[$estilo,'P','D','','CS']);
					?>

					<div id="spaceToAddOpe">
					<?php foreach($responseDataSIGEOperaciones as $fila): ?>
						<?php 

						$style='padding: 2px 5px 8px 5px;font-size:10px;';
								if(strlen ($fila->{'Des_Operacion_Costura'})<16){
									$style="";
								}
						?>


						<div class="sameline classOperacion" data-codope="<?=  $fila->{'cod_operacion_costura'}; ?>">
							<div class="itemsFinalTbl" style="width: 60px;"><?=  $fila->{'cod_operacion_costura'}; ?></div>
							<div class="itemsFinalTbl alignLeft maxHeight" style="width: 120px;<?= $style; ?>"><?= $fila->{'Des_Operacion_Costura'}; ?></div>
						</div>
				...
				<div class="placeDefectos">
					...
					<div class="spaceFlex" id="spaceToBtn">
						...
						?>
							...
							<?php
								for ($j=0; $j < count($responseDataSIGEOperaciones) ; $j++) { 
							?>	
									<div class="itemsFinalTbl maxHeight counterBtn" style="width: 88px;">
										<div class="divBtnsAddMinus" data-def="<?php echo $defectos[$i]['CODDEF']; ?>"
											data-ope="<?php echo $responseDataSIGEOperaciones[$j]->{'cod_operacion_costura'}; ?>"
											onclick="addValue('<?php echo $responseDataSIGEOperaciones[$j]->{'cod_operacion_costura'}; ?>',<?php echo $defectos[$i]['CODDEF']; ?>)"
											id="<?php echo 'OPE'.$responseDataSIGEOperaciones[$j]->{'cod_operacion_costura'}.'DEF'.$defectos[$i]['CODDEF']; ?>">0</div>
										<div class="buttonminus" onclick="desValue(<?php echo $responseDataSIGEOperaciones[$j]->{'cod_operacion_costura'}; ?>,<?php echo $defectos[$i]['CODDEF']; ?>)"><i class="fa fa-minus" aria-hidden="true"></i></div>
									</div>
							<?php
								}
							?>
				...
				<div class="placeTotales">
					...
					<?php
						for ($j=0; $j < count($responseDataSIGEOperaciones) ; $j++) { 
					?>	
						<div class="itemsFinalTbl itemFinalTotal" id="ope<?php echo $responseDataSIGEOperaciones[$j]->{'cod_operacion_costura'}; ?>">0</div>
					<?php
						}
					?>
					</div>
	...
	<div class="selectionOpeModal" style="display: none;">
		<div class="bodyModal">
			...
			<select id="seleccionope" class="selectclass" style="width: calc(100% - 2px);margin-bottom: 5px; border: 1px solid #666;">
				<?php
				for ($i=0;$i<count($responseDataSIGEOperaciones);$i++) {
				?>
				<option value="<?php echo $responseDataSIGEOperaciones[$i]->{'cod_operacion_costura'}; ?>"><?php echo $responseDataSIGEOperaciones[$j]->{'Des_Operacion_Costura'}; ?></option>
				<?php
				}
				?>
			</select>
