<?php
	...
	if (!isset($_POST['codinscos'])) {
		...
		if ($codinscos!="0") {
			...
			$sql="BEGIN SP_INSP_INSERT_INSCOS_V2(:CODUSU,:CODFIC,:TURNO,:CANPRE,:CANPREDEF,:CODTLL,:CODINSCOS,:CANTIDAD); END;";
			$stmt=oci_parse($conn, $sql);
			oci_bind_by_name($stmt, ':CODUSU', $_POST['codusu']);
			oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
			...
			oci_bind_by_name($stmt, ':CANTIDAD', $cantidadreturn,40);

			$result=oci_execute($stmt);

			if ($result) {
				$response->cantidadreturn=$cantidadreturn;
				$response->codinscos=$codinscos;
				$response->state=true;
		...
		$sql="BEGIN SP_INSP_UPDATE_INSCOS_V2(:CODINSCOS,:CANPRE,:CANPREDEF,:CANTIDAD); END;";
		$stmt=oci_parse($conn, $sql);
