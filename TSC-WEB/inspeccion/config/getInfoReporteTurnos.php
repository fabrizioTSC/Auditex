<?php
	...
	$stmt = oci_parse($conn,'BEGIN SP_GET_HORAACTUAL(:HORACT); END;');
	$OUTPUT_CUR=oci_new_cursor($conn);
	oci_bind_by_name($stmt,':HORACT',$horact,40);
	$result=oci_execute($stmt);
	$horact=intval($horact);
	...
	while($row=oci_fetch_assoc($OUTPUT_CUR)){
		...
		if ($row['TURNO']==$turnoant) {
			/*$minefit+=floatval(str_replace(",",".",$mineficienciacom));
			$minefct+=floatval(str_replace(",",".",$mineficaciacom));*/
			$minefit+=floatval(str_replace(",",".",$mineficienciacomest));
			$minefct+=floatval(str_replace(",",".",$mineficaciacomest));
			$minasi1t+=floatval(str_replace(",",".",$minasi1));
			$minasi2t+=floatval(str_replace(",",".",$minasi2));
			$predeft+=$prendasdef;
			$preinst+=$prendasins;
			$prerept+=$prendasrep;
		}else{
