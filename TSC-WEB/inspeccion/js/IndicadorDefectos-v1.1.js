			...
			for (var i = 0; i < data.defectos.length; i++) {
				...
				'<div class="lineBody" onclick="detalle_defecto(\''+data.defectos[i]['CODDEF']+'\',\''+data.defectos[i]['DESDEF']+'\')">'+
					'<div class="itemhs1 items4" style="width: calc(55% - 10px);text-align:left;display:flex;">'+
						'<div id="ext-'+data.defectos[i]['CODDEF']+'" class="ext-def" onclick="show_ext_def(\''+data.defectos[i]['CODDEF']+'\',\''+data.defectos[i]['DESDEF']+'\')">+</div>&nbsp;'+
						(i+1)+'. '+data.defectos[i]['DESDEF']+
					'</div>'+
				...
				'</div>'+
				'<div class="lineBody line-ext" id="ext-body-'+data.defectos[i]['CODDEF']+'" style="display:none;">'+
				'</div>';
			}
			$("#idDefectos").empty();
...
function detalle_defecto_taller(codtll_new,destll){
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#titulodetalle").empty();
			$("#titulodetalle").append("Linea: "+codtll_new);
...
function detalle_defecto(coddef,desdef){
	...
	$.ajax({
		...
		success:function(data){
			...
			param_codran1=parseInt(data.param[1].VALOR);
			$("#titulodetalle").empty();
			$("#titulodetalle").append("Linea: "+codlin);
