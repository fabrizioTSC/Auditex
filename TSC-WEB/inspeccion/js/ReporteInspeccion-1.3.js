...
$(document).ready(function(){
	...
	$("#selectOperaciones").change(function(){
		if (document.getElementById("check1").checked) {
			...
			$.ajax({
				type:"POST",
				url:"config/getDetalleOperacionDefTur.php",
				data:{
					turno:$("#select-turno").val(),
					codope:$("#selectOperaciones").val()
				},
				success:function(data){
					//console.log(data);
	...
	$("#selectDefectos").change(function(){
		if (document.getElementById("check1").checked) {
			...
			$.ajax({
				type:"POST",
				url:"config/getDetalleDefectosOpeTur.php",
				data:{
					turno:$("#select-turno").val(),
					coddef:$("#selectDefectos").val()
				},
	...
	$("#selectDefectosMaq").change(function(){
		if (document.getElementById("check1").checked) {
			...
			$.ajax({
				type:"POST",
				url:"config/getDetalleDefectosOpeMaqTur.php",
				data:{
					turno:$("#select-turno").val(),
					coddef:$("#selectDefectosMaq").val()
				},
	...
	$.ajax({
		...
		success:function(data){
			...
			for (var i = 0; i < data.turnos.length; i++) {
				html+='<option value="'+data.turnos[i].TURNO+'">'+data.turnos[i].TURNO+'</div>';
			}
			$("#select-turno").append(html);
...
function showReporte(){
	...
	if (document.getElementById("check1").checked) {
		...
		if (linea=="" || codtll_var==0 || codtll_var=="") {
			...
			$("#idTurnoTur").text($("#select-turno").val());
			$.ajax({
				type:"POST",
		...
		$.ajax({
			type:"POST",
			url:"config/getLineaInfoTur.php",
			data:{
				turno:$("#select-turno").val()
			},
			success:function(data){
				console.log(data);
				if (data.detDefecto.detalle1.length!=0) {
					showGraph(data);
					interval=setInterval(function(){
						chargeChart();
					},tiempo*1000*60);
...
function showGraph(data){
	...
	$("#idFecha").text(fechaInicio + " - " + fechaFin);
	$("#idFechaTur").text(hoy);
...
function downloadPDF(){
	if (codtll_var=="" || codtll_var==null || codtll_var==undefined) {
		codtll_var=$("#select-turno").val();
	}
	$.ajax({
	  	type: "POST",
	  	...
	  	success:function(data){
	  		...
	  		var a=document.createElement("a");
	  		a.target="_blank";
	  		var dir="fpdf/pdfReporteMonitor.php?"+
	  			"n="+data+
	  			"&cf="+codfic+
	  			"&dc="+descli+
	  			"&p="+pedido;
			if (document.getElementById("check1").checked) {
				dir+="&nt="+nomtll+"&ct="+codtll_var;
			}else{
				dir+="&turno="+$("#select-turno").val();
			}
	  		a.href=dir;
	  		a.click();
	  	}
