	...
	for (var i = 0; i < data.length; i++) {
		html+=
			'<div class="tbl-linea">'+
				...
				'<div class="body-tbl tbl-in3" style="padding-top:8px;">'+data[i].RUTA+'</div>'+
				'<div class="body-tbl tbl-in3" style="padding-top:8px;"><span id="std-'+data[i].CODFIC+'">'+data[i].TIE_STD+'</span></div>'+
				'<div class="body-tbl tbl-in3" style="padding-top:8px;"><span id="estcom-'+data[i].CODFIC+'">'+data[i].TIE_COM_EST+'</span></div>'+
...
function recalc_tot(dom){
	...
	$("#tot-"+codfic).text(Math.round((parseFloat($("#std-"+codfic).text())+
		parseFloat($("#estcom-"+codfic).text())+
		parseFloat(dom.value))*1000)/1000);
}
