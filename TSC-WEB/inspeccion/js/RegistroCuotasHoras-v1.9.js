	...
	$.ajax({
		...
		success:function(data){
			...
			for (var i = 0; i < data.fechas.length; i++) {
				html+=
				'<option value="'+data.fechas[i].CODFEC+'">'+data.fechas[i].FECHA+'</option>';
			}
			$("#select-fechas").empty();
			$("#select-fechas").append(html);

			var html='';
			for (var i = 0; i < data.lineas.length; i++) {
				// html+='<option value="'+data.lineas[i].LINEA+'">LINEA '+data.lineas[i].LINEA+'</option>';
	...
	for (var i = 0; i < data.length; i++) {
		html+=
			'<div class="tbl-linea">'+
				...
				'<div class="body-tbl tbl-in3"><input type="number" class="simple-ipt ipt-mindes" data-hora="'+data[i].HORA+'" id="mindes-'+data[i].HORA+'" value="'+data[i].MIN_DESCUENTO+'"/></div>'+
				'<div class="body-tbl tbl-in3"><input type="number" class="simple-ipt ipt-totmin" id="minutos-'+data[i].HORA+'" value="'+data[i].MINASI+'" disabled/></div>'+
			...
			'</div>';
		totminasi+=parseFloat(data[i].MINASI);
	}
	document.getElementById("total-minutos").innerHTML=totminasi;
	html+=
	'<script>'+
...
function correct_total_min(){
	var totminasi=0;
	var ar=document.getElementsByClassName("ipt-totmin");
	for (var i = 0; i < ar.length; i++) {
		totminasi+=parseFloat(ar[i].value);
	}
	document.getElementById("total-minutos").innerHTML=totminasi;
}
...
function recalc_min(element){
	...
	$("#minutos-"+hora).val(parseInt($("#minhor-"+hora).val())*parseInt($("#numope-"+hora).val())-parseInt($("#mindes-"+hora).val()));
}
