<?php
	...
	for ($i=0; $i < count($array); $i++) { 
		$sql="BEGIN SP_AA_UPDATE_CHELISACA(:CODFIC,:PARTE,:NUMVEZ,:CODPRO,:LLEVA,:RESPRO); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		oci_bind_by_name($stmt, ':PARTE', $_POST['parte']);
		oci_bind_by_name($stmt, ':NUMVEZ', $_POST['numvez']);
		oci_bind_by_name($stmt, ':CODPRO', $array[$i][0]);
		oci_bind_by_name($stmt, ':LLEVA', $array[$i][1]);
		oci_bind_by_name($stmt, ':RESPRO', $array[$i][2]);
		$result=oci_execute($stmt);
		if (!$result) {
			$con_err++;
		}
