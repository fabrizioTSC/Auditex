<?php
	...
	$sql="BEGIN SP_PCVA_INSERT_DEFCAJVEZ(:PEDIDO,:COLOR,:PARTE,:NUMVEZ,:NUMCAJERP,:NUMVEZCAJA,:NUMVEZLOTE,:CODDEF,:CANDEF); END;";
	...
	if($result){
		...
		$detalle_defecto=[];
		$sql="BEGIN SP_PCVA_SELECTAUDCAJADEFVEZ(:PEDIDO,:COLOR,:PARTE,:NUMVEZ,:NUMVEZLOTE,:NUMCAJERP,:NUMVEZCAJA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt,':PEDIDO',$_POST['pedido']);
		oci_bind_by_name($stmt,':COLOR',$_POST['color']);
		oci_bind_by_name($stmt,':PARTE',$_POST['parte']);
