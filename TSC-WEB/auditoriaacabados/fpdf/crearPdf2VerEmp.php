...
$por_rec[0]="% RECH.";
$pre_apr=[];
$pre_apr[0]="CAJA APRO. 1RA";
$pre_rec=[];
$pre_rec[0]="CAJA RECH.";
$pre_tot=[];
$pre_tot[0]="TOTAL CAJA";
$por_pre_apr=[];
$por_pre_apr[0]="% APRO. 1RA";
$por_pre_rec=[];
$por_pre_rec[0]="% RECH.";
...
$pre_prips=[];
$pre_prips[0]="CAJA APR. 1RA";
$pre_segps=[];
$pre_segps[0]=utf8_decode("CAJA APR. 2DA Ó MÁS");
$pre_totps=[];
$pre_totps[0]="TOTAL CAJA APR.";
$por_pre_prips=[];
...
$sql="BEGIN SP_AA_INDICADOR_VEREMPACA(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:OPCION,:OUTPUT_CUR); END;";
...
while ($row=oci_fetch_assoc($OUTPUT_CUR)) {
	$headers[$l]=$row['ANHO'];
	$aud_apr[$l]=number_format($row['AUD_APR1']);
	$aud_rec[$l]=number_format($row['AUD_REC']);
	$aud_tot[$l]=number_format($row['AUD_IND']);
	$por_apr[$l]=(round($row['AUD_APR1']*10000/$row['AUD_IND'])/100)."%";
	$por_rec[$l]=(round($row['AUD_REC']*10000/$row['AUD_IND'])/100)."%";
	$pre_apr[$l]=number_format($row['PREN_APR1']);
	$pre_rec[$l]=number_format($row['PREN_REC']);
	$pre_tot[$l]=number_format($row['PREN_IND']);
	$por_pre_apr[$l]=(round($row['PREN_APR1']*10000/$row['PREN_IND'])/100)."%";
	$por_pre_rec[$l]=(round($row['PREN_REC']*10000/$row['PREN_IND'])/100)."%";

	$headersps[$l]=$row['ANHO'];
	$aud_prips[$l]=number_format($row['AUD_APR1']);
	$aud_segps[$l]=number_format($row['AUD_APR2']);
	$aud_totps[$l]=number_format($row['AUD_APR1']);
	$por_prips[$l]=(round($row['AUD_APR1']*10000/$row['AUD_APR1'])/100)."%";
	$por_segps[$l]=(round($row['AUD_APR2']*10000/$row['AUD_APR1'])/100)."%";
	$pre_prips[$l]=number_format($row['PREN_APR1']);
	$pre_segps[$l]=number_format($row['PREN_APR2']);
	$pre_totps[$l]=number_format($row['PREN_APR1']);
	$por_pre_prips[$l]=(round($row['PREN_APR1']*10000/$row['PREN_APR1'])/100)."%";
	$por_pre_segps[$l]=(round($row['PREN_APR2']*10000/$row['PREN_APR1'])/100)."%";
	$por_totps[$l]="100%";
	$l++;
}
...
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("INDICADORES DE RESULTADOS DE AUDITORÍA DE"),0,0,'C');
$pdf->Ln();
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("VERIFICADO DE EMPAQUE ACABADOS - GENERAL"),0,0,'C');
$pdf->Ln();
...
for ($i=0; $i < count($por_apr); $i++) { 
	if ($i==0) {
		...
		if ($porcen<intval($_GET['parran2'])) {			
			$pdf->SetFillColor(220,20,30);
		}else{
			if($porcen>=intval($_GET['parran1'])){
...
for ($i=0; $i < count($pre_apr); $i++) { 
	if ($i==0) {
		...
		$pdf->Cell($width_by_cell,6,$pre_apr[$i],1,0,'R');
	}
}
$pdf->Ln();
for ($i=0; $i < count($pre_rec); $i++) { 
	if ($i==0) {
		...
		$pdf->Cell($width_by_cell,6,$pre_rec[$i],1,0,'R');
	}
}
$pdf->Ln();
for ($i=0; $i < count($pre_tot); $i++) { 
	if ($i==0) {
		...
		$pdf->Cell($width_by_cell,6,$pre_tot[$i],1,0,'R');
	}
}
$pdf->Ln();
$pdf->SetFillColor(10,70,150);
for ($i=0; $i < count($por_pre_apr); $i++) { 
	if ($i==0) {
		...
		$pdf->Cell($width_by_cell,6,$por_pre_apr[$i],1,0,'R');
	}
}
$pdf->Ln();
$pdf->SetFillColor(10,70,150);
for ($i=0; $i < count($por_pre_rec); $i++) { 
	if ($i==0) {
		...
		$pdf->SetTextColor(0);
		$pdf->Cell($width_by_cell,6,$por_pre_rec[$i],1,0,'R');
	}
}
...
$i=0;
$sumDefectosU=0;
$sql="BEGIN SP_AA_INDICADOR_VEREMPDET(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:CODFAM,:OPCION,:OUTPUT_CUR); END;";
$stmt=oci_parse($conn, $sql);
oci_bind_by_name($stmt, ':CODTLL', $_GET['ct']);
oci_bind_by_name($stmt, ':CODSED', $_GET['cs']);
oci_bind_by_name($stmt, ':CODTIPSER', $_GET['cts']);
...
$pdf->Output('D','Indicar de Resultados Verificado de Empaque - '.str_replace("/","-",$_GET['t']).'.pdf',true);
//$pdf->Output('','',true);
