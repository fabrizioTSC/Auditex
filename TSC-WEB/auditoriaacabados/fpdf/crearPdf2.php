...
$sql="BEGIN SP_AA_INDICADOR_CLAUX(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:OPCION,:OUTPUT_CUR); END;";
...
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("INDICADORES DE RESULTADOS DE CHECK LIST PROCESO ACABADOS - GENERAL"),0,0,'C');
$pdf->Ln();
...
$i=0;
$sumDefectosU=0;
$sql="BEGIN SP_AA_INDICADOR_CLDET(:CODTLL,:CODSED,:CODTIPSER,:FECHA,:CODFAM,:OPCION,:OUTPUT_CUR); END;";
$stmt=oci_parse($conn, $sql);
oci_bind_by_name($stmt, ':CODTLL', $_GET['ct']);
oci_bind_by_name($stmt, ':CODSED', $_GET['cs']);
oci_bind_by_name($stmt, ':CODTIPSER', $_GET['cts']);
...
for ($i=0; $i < count($defuno); $i++) { 
	...
	$porcen=round($defuno[$i]->SUMA*10000/ ($sumDefectosD > 0 ? $sumDefectosD : 1 ) )/100;
