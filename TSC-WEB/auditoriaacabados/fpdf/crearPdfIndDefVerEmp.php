...
$pdf->Cell($pdf->get_max_width_to_white(),6,utf8_decode("INDICADORES DE DEFECTOS DE AUDITORÍA DE VERIFICADO DE EMPAQUE ACABADOS"),0,0,'C');
$pdf->Ln();
...
$sql="BEGIN SP_AA_INDVEREMP_DEFECTOS(:CODTLL,:CODSED,:CODTIPSER,:OPCION,:CODDEF,:OUTPUT_CUR); END;";
...
while ($row=oci_fetch_assoc($OUTPUT_CUR)) {
	...
	$num_can_mue[$l]=number_format($row['SUMCANMUE']);
	$num_def_tot[$l]=number_format($row['SUMDEF']);
	$por_def[$l]=(round($row['SUMCODDEF']*10000/$row['SUMCANMUE'])/100)."%";
	$l++;
}
...
$pdf->Ln();
for ($i=0; $i < count($num_can_mue); $i++) { 
	if ($i==0) {
		$pdf->Cell($first_col_width,6,$num_can_mue[$i],1,0,'L',true);
	}else{
		$pdf->Cell($width_by_cell,6,$num_can_mue[$i],1,0,'R');
	}
}
$pdf->Ln();
...
$pdf->Output('D','Indicar de Defectos Verificado de Empaque Acabadados - '.str_replace("/","-",$_GET['t']).'.pdf',true);
