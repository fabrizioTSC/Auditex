...
$(document).ready(function(){
	$.ajax({
		...
		url:"config/getInfoIndDefVerEmp.php",
	...
	$("#idNumAnio").change(function(){
		...
		$.ajax({
			...
			url:"config/getSemanasIndDefVerEmp.php",
			success:function(data){
				console.log(data);
				var html='';
				for (var i = 0; i < data.semanas.length; i++) {
	...
	$("#idNumSem").change(function(){
		...
		$.ajax({
			...
			url:"config/getNewInfoIndDefVerEmp.php",
			success:function(data){
				console.log(data);
				fill_defectos(data.defectos);
				$(".panelCarga").fadeOut(200);
...
function detalle_defecto(coddef,desdef){
	...
	$.ajax({
		...
		url:"config/getInfoIndDefXCodVerEmp.php",
		success:function(data){
			console.log(data);
			...
			for (var i = 0; i < data.anios.length; i++) {
				...
				if (parseInt(data.anios[i]['SUMDEF'])!=0) {
					//porcentaje=Math.round(Math.round(parseInt(data.anios[i]['SUMCODDEF'])*10000)/parseInt(data.anios[i]['SUMDEF']))/100;
			...
			for (var i = 0; i < data.meses.length; i++) {
				...
				if (parseInt(data.meses[i]['SUMDEF'])!=0) {
					//porcentaje=Math.round(Math.round(parseInt(data.meses[i]['SUMCODDEF'])*10000)/parseInt(data.meses[i]['SUMDEF']))/100;
			...
			for (var i = 0; i < data.semanas.length; i++) {
				...
				if (parseInt(data.semanas[i]['SUMDEF'])!=0) {
					//porcentaje=Math.round(Math.round(parseInt(data.semanas[i]['SUMCODDEF'])*10000)/parseInt(data.semanas[i]['SUMDEF']))/100;
...
function downloadPDF(){
	$.ajax({
	  	...
	  	success:function(data){
	  		...
	  		a.target="_blank";
	  		a.href="fpdf/crearPdfIndDefVerEmp.php?n="+data+
	  		"&t="+title+"&t2="+title2+
...
function update_fechas(){
	...
	$.ajax({
		...
		url:"config/getInfoIndDefXFecVerEmp.php",
		success:function(data){
			console.log(data);

			fill_defectos(data.defectos,data.sumtot);	
...
function show_ext_def(coddef,desdef){
	...
	if (tipo==1) {
		$.ajax({
			...
			url:"config/getInfoIndDefTllVerEmp.php",
			success:function(data){
				console.log(data);
