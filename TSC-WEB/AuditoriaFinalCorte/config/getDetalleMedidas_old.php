<?php
	...
	if ($result) {
		...
		$ant_nom="";
		$cont=0;
		$sql="BEGIN SP_AFC_SELECT_ESTILOMEDIDA_JV2(:CODFIC,:HILO,:TRAVEZ,:LARGMANGA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		...
		$i=0;
		$validador=false;
		$sql="BEGIN SP_AFC_SELECT_HEAMEDTALV2(:CODFIC,:HILO,:TRAVEZ,:LARGMANGA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		oci_bind_by_name($stmt, ':HILO', $_POST['hilo']);
		oci_bind_by_name($stmt, ':TRAVEZ', $_POST['travez']);
		...
		$i=0;
		$validador=false;
		$sql="BEGIN SP_AFC_SELECT_FICCORMEDFIN_JV2(:CODFIC,:HILO,:TRAVEZ,:LARGMANGA,:OUTPUT_CUR); END;";
		$stmt=oci_parse($conn, $sql);
		oci_bind_by_name($stmt, ':CODFIC', $_POST['codfic']);
		oci_bind_by_name($stmt, ':HILO', $_POST['hilo']);
		oci_bind_by_name($stmt, ':TRAVEZ', $_POST['travez']);
