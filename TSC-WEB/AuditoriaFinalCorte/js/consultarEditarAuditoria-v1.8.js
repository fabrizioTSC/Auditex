	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.state==true){
				...
				// encogimeintos
				let hilo = data.ficha.MOLDE_USAR_HILO != null ? parseFloat(data.ficha.MOLDE_USAR_HILO) : "";
				let trama = data.ficha.MOLDE_USAR_TRAMA != null ? parseFloat(data.ficha.MOLDE_USAR_TRAMA) : "";
				let manga = data.ficha.MOLDE_USAR_MANGA != null ? parseFloat(data.ficha.MOLDE_USAR_MANGA) : "";


				let htmlnew = '<div class="tblLine">'+
						'<div class="itemBody" style="width: 33.33%;">'+hilo+'%</div>'+
						'<div class="itemBody" style="width: 33.33%;">'+trama+'%</div>'+
						'<div class="itemBody" style="width: 33.34%;">'+manga+'%</div>'+
					'</div>';

				$("#idTblEnco").html(htmlnew);
