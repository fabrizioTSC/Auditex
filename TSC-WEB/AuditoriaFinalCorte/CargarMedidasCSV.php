...
<body>
	...
	<div class="mainContent">
		...
		<div class="bodyContent mainBodyContent">	
			<div class="btn-primary" onclick="click_input()" style="margin:0px;">Cargar CSV</div>
	...
	<script type="text/javascript">
		...
		function info_to_save(esttsc){
			$.ajax({
				...
				success:function(data){
					...
					var c;
					if (!validador2) {
						c=confirm(`Ya existen medidas para el ESTILO => ${esttsc} - HILO => ${hilo} - TRAVEZ => ${travez} - LARG. MANGA =>${largmanga} . Desea borrar y cargar las medidas nuevamente?`);
						if (c) {
							validador2=true;
						}else{
							...
							success:function(data){
								...
								if (data.state) {
									...
									if(data.detail==undefined){
										alert("Problemas con la carga. Indique el procedimiento realizado para corregir el aplicativo al equipo de desarrollo.");
									}else{
										alert(data.detail);
									}
									cargado="N";
								}
								carga_csv_file(esttsc,hilo*val_round,travez*val_round,largmanga*val_round,cargado);
								if (pos_send==ar_esttsc.length-1) {
									console.log("hola");
		...
		function carga_csv_file(esttsc,hilo,travez,largmanga,cargado){
			var fd=new FormData();
			fd.append('file',f);
			fd.append('esttsc',esttsc);
			fd.append('hilo',hilo);
			fd.append('travez',travez);
			fd.append('largmanga',largmanga);
			fd.append('cargado',cargado);
			var request=new XMLHttpRequest();
			request.open('POST','config/__save_csv_file.php',false);
		        ...
		        r.onload = function(e){
		            ...
		            while(validador==false && i<pos_start_medida){
		            	...
		            	while (k < ar_aux.length && val_med_2==false) {
							...
							if((ar_aux[k].toUpperCase()).indexOf("TRAVEZ")>=0){
			            		...
			            		while ( validador2==false && j< ar_aux.length) {
			            			if(ar_aux[j]!=""){
										travez = ar_aux[j].replace("%","").replace(",",".");
							...
							if((ar_aux[k].toUpperCase()).indexOf("LARG.DE MANGA")>=0){
			            		...
			            		while ( validador2==false && j< ar_aux.length) {
			            			if(ar_aux[j].trim()!=""){
										largmanga = ar_aux[j].replace("%","").replace(",",".");
										validador2=true;
										islarman=1;
			            			}
			            			j++;
		            ...
		            if (esttsc=="") {
			            ...
			            if (val_tam_text) {
							...
							// AGREGADO
							let esttscsige = ar_est.length > 0 ? ar_est[0] : null;
							if(esttscsige != null && esttscsige.includes('-')){
								new_ar = esttscsige;
							}else{
								for (var i = 0; i < ar_est.length; i++) {
									//console.log(ar_est[i]);
