...
<body>
	...
	<div class="mainContent">
		...
		<div class="bodyContent mainBodyContent">	
			<div class="btn-primary" onclick="click_input()" style="margin:0px;">Cargar Archivo</div>
			<div style="display: none;" id="show_preview">
				<div class="lineDecoration"></div>
				<div class="lbl">Estilo: <span id="idesttsc"></span></div>	
				<div id="maintbl" style="margin-bottom: 10px;overflow-x: scroll;height: calc(100vh - 200px);position: relative;">
	...
	<form method="POST" action="config/saveXLSX.php" style="display: none;" enctype='multipart/form-data'>
		<input type="file" id="inputFile" name="file">
		<input type="submit" id="inputSubmit" >
	</form>
	<script type="text/javascript">
		var file_name="<?php if (isset($_GET['file'])) { echo $_GET['file'];}else { echo ""; } ?>"
		$(document).ready(function(){
			...
			if (file_name!="") {
				$.ajax({
			        type: "GET",
			        url: "config/CSV_tmp/"+file_name,
			        dataType: "text",
			        success: function(data) {
			        	//console.log(data);
			        	var delimiter=counter_delimiter(data);
			            var ar1=data.split("\n");
			            ...
			            var pos_enter=data.indexOf("TOL\n(-)");
			            if (pos_enter>=0) {
							search_enter=true;
			            }
			        ...
			        error:function(error){
						...
						window.location.href="CargarMedidas.php";
			        }
			    });
			}else{
				$(".panelCarga").fadeOut(100);
		...
		function send_data(){
            ...
            var ar_esttsc=esttsc.split(";");
            console.log(ar_esttsc);
            //for (var i = 0; i < ar_esttsc.length; i++) {
				$.ajax({
					type:"POST",
					data:{
						esttsc:ar_esttsc[0]
						...
						if (!validador2) {
							...
							if (c) {
	            				...
	            				$.ajax({
									...
									url:"config/deleteCSVSinUso.php",
									success:function(data){
										if (data.state) {
											window.location.href="CargarMedidas.php";
										}
						...
						if(validador2){
							$.ajax({
								...
								success:function(data){
									console.log(data);
									if (data.state) {
										alert("Se cargaron "+data.insert1+" medidas a ESTILOTSCMEDIDA y "+data.insert2+" registros de detalle a ESTILOTSCMEDIDADETALLE!");
										window.location.href="CargarMedidas.php";
									}else{
										alert(data.detail);
									}
