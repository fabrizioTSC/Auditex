...
$(document).ready(function(){
	$.ajax({
		...
		success:function(data){
			...
			if (codtipser=="0" && codsede=="0") {
				...
				for (var i = 0; i < data.anios.length; i++) {
					...
					'<div class="divanios[i]">'+
						...
						'<div class="itemhs1 items4" style="background: yellow;">'+porcentaje+'%</div>'+
			...
			$("#titParUno").text(data.semanas[data.semanas.length-1]['ANIO']+" - Semana "+data.semanas[data.semanas.length-1]['NUMERO_SEMANA']);
			$("#titParDos").text(data.semanas[data.semanas.length-1]['ANIO']+" - Semana "+data.semanas[data.semanas.length-1]['NUMERO_SEMANA']);
			$("#titParUno-Mes").text(data.meses[data.meses.length-1]['ANIO'].substring(0,4)+" - "+proceMesLarge(data.meses[data.meses.length-1]['MES']));
			$("#titParDos-Mes").text(data.meses[data.meses.length-1]['ANIO'].substring(0,4)+" - "+proceMesLarge(data.meses[data.meses.length-1]['MES']));
		...
		options: {
			...
			plugins: {
				datalabels: {
					...
					formatter: function(value, context) {
						if (context.dataset.type=="bar") {
							return Math.round(value*100/sumTotMes)+"%";
	                    }else{
		...
		options: {
			...
			plugins: {
				datalabels: {
					...
					formatter: function(value, context) {
						if (context.dataset.type=="bar") {
							return Math.round(value*100/sumTotUSem)+"%";
	                    }else{
							return Math.round(value)+"%";                  	
	                    }
					},
		...
		options: {
			...
			plugins: {
				datalabels: {
					...
					formatter: function(value, context) {
						if (context.dataset.type=="bar") {
							return Math.round(value*100/sumTotUMes)+"%";
	                    }else{
							return Math.round(value)+"%";               	
	                    }
					},
		...
		options: {
			...
			plugins: {
				datalabels: {
					...
					formatter: function(value, context) {
						if (context.dataset.type=="bar") {
							return Math.round(value*100/sumTotDSem)+"%";
	                    }else{
							return Math.round(value)+"%";              	
	                    }
					},
		...
		options: {
			...
			plugins: {
				datalabels: {
					...
					formatter: function(value, context) {
						if (context.dataset.type=="bar") {
							return Math.round(value*100/sumTotDMes)+"%";
	                    }else{
							return Math.round(value)+"%";              	
	                    }
					},
...
function downloadPDF(){
	$.ajax({
	  	...
	  	data: { 
	    	imgBase64: document.getElementById("chart-area-ps").toDataURL("image/png"),
	  	...
	  	success:function(data){
	  		...
	  		"&t4="+t4+"&t4_1="+t4_1+"&t4_2="+t4_2+"&t5="+t5+"&t5_1="+t5_1+"&t5_2="+t5_2+"&cts="+codtipser+"&cs="+codsede+"&fecha="+fecha;
	  		a.click();
	  	}
	}).done(function(o) {
