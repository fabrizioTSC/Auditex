...
function cargarDatos(codfic,codusu){
	...
	$.ajax({
		...
		url:"config/getDetalleFichaAPNCCon.php",
		success:function(data){
			...
			if (data.state) {
				...
				document.getElementById("idnomtal").innerHTML='<a href="../auditex/ConsultarEditarAuditoria.php?solocorte=1&codfic='+codfic+'">'+val_u(data.partida.tallercos)+'</a>';
				...
				if (data.partida.tallercor!=null) {
					...
					'<a href="../AuditoriaFinalCorte/ConsultarEditarAuditoria.php?solocorte=1&codfic='+
				...
				for (var i = 0; i < data.detfictal.length; i++) {
					...
					'<tr>'+
						...
						'<td><div class="div-inline">'+
							...
							data.detfictal[i].CODDEF+'-'+data.detfictal[i].CODTAL+'-'+
							data.detfictal[i].UBIDEF+'" data-obj="canfin1">'+data.detfictal[i].CANFIN1+'</span>'+
						...
						data.detfictal[i].CODDEF+'-'+data.detfictal[i].CODTAL+'-'+
						data.detfictal[i].UBIDEF+'" value="'+data.detfictal[i].OBS+'"/></td>'+
					'</tr>';
				}
				html+=
					'<tr>'+
