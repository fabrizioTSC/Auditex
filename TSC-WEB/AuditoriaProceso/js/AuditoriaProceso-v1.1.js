...
function pass_addOperacion(){
	if(codtipser_tll=="2"||codtipser_tll=="1"){
		if (codope_var==0||codope_var=="") {
			...
			let cantpren = $("#txtcantprendas").val();

			if(cantpren != "" && cantpren != "0" ){
				...
				alert("Debe agregar la cantidad de prendas");
			}
		...
		$.ajax({
			...
			success:function(data){
				...
				if (data.state==true) {
					...
					html+=
					'<div class="tblLine">'+
						'<div class="itemBody" style="width: 50%;">'+wordDef_ft+'</div>'+
						'<div class="itemBody" style="width: 25%;">'+$("#idCanDef").val()+'</div>'+
						'<div class="itemBody" style="width: 25%;">'+$("#txtobservacion").val()+'</div>'+
					'</div>';
