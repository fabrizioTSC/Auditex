<?php
	...
	if (isset($_POST['fichas'])) {
		...
		$sqlUpdate="UPDATE fichacosturaavance set ESTADO='I',FECINI=CURRENT_DATE".
		...
		if ($result) {
			...
			while($row=oci_fetch_array($stmt,OCI_ASSOC)){
				$fichasArray[$i]=" INTO fichacosturamovimiento values ('".$row['CODFIC']."',".$row['PARTE'].",CURRENT_DATE,'".$_POST['usumov']."',".
				"2,".$row['CANPAR'].",'".$row['CODTLL']."','A')";
				$i++;
			}
			$parteSql="";
